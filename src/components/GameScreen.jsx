...
function GameScreen({ playerName, onGameEnd }) {
  ...
  const getButtonClass = (index) => {
    const baseClass = "w-full p-4 rounded-xl text-left font-semibold transition-all duration-300 transform hover:scale-105"
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-6 md:p-10">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold text-kenya-green">{score} / {questions.length}</p>
          </div>
        </div>
      ...
      {/* Result Feedback */}
      {showResult && (
        <div className={`p-4 rounded-xl mb-6 ${answeredCorrectly ? 'bg-green-100 border-2 border-green-500' : 'bg-red-100 border-2 border-red-500'}`}>
          <p className={`text-center font-bold text-lg ${answeredCorrectly ? 'text-green-700' : 'text-red-700'}`}>
            {answeredCorrectly ? '🎉 Correct! Well done!' : '❌ Incorrect! Better luck next time!'}
          </p>
        </div>
      )}
